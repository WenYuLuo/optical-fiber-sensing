...
class GMMHMM(_BaseHMM):
    ...
    def _init(self, X, lengths=None):
        ...
        self.post_comp_mix = np.zeros((_n_samples, self.n_components, self.n_mix))
    ...
    def _accumulate_sufficient_statistics(self, stats, X, framelogprob,
        ...
        # org_V = stats['samples']
        # if np.any(org_V == None):
        #     stats['samples'] = X
        # else:
        #     stats['samples'] = np.vstack((org_V, X))
        ...
        # org_V = stats['post_comp_mix']
        # if np.any(org_V == None):
        #     stats['post_comp_mix'] = post_comp_mix
        # else:
        #     stats['post_comp_mix'] = np.vstack((org_V, post_comp_mix))
        temp_index = stats['n_samples']
        self.post_comp_mix[temp_index:(temp_index+n_samples)] = post_comp_mix
        stats['n_samples'] += n_samples

        stats['post_mix_sum'] += np.sum(post_comp_mix, axis=0)
        stats['post_sum'] += np.sum(post_comp, axis=0)

        # centered_X = X[:, np.newaxis, np.newaxis, :] - self.means_
        # org_V = stats['centered']
        # if np.any(org_V == None):
        #     stats['centered'] = centered_X
        # else:
        #     stats['centered'] = np.vstack((org_V, centered_X))
    ...
    def _do_mstep(self, stats):
        ...
        stats['post_comp_mix'] = self.post_comp_mix
        ...
        stats['centered'] = stats['samples'][:, np.newaxis, np.newaxis, :] - new_means
